...
from polp.datasets.code_code import Devign
from torch.utils.data import DataLoader
from polp.nn.layers import AttentionMask
from pruning.classifier import Model
...
def train(args, model, tokenizer):
    ...
    for idx in range(0, int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(bar):
            code, labels = batch['function_body'], batch['label']
            input_sentence = tokenizer(code, padding='longest', truncation=True, max_length=512)
            ids = torch.tensor(input_sentence['input_ids'])
            ids = ids.to(args.device)
            mask = torch.tensor(input_sentence['attention_mask']).bool()
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                #         logger.info(f"args.evaluate_during_training : true")
                #
                #     logger.info("  " + "*" * 20)
                ...
                #         logger.info(f"[DEBUG] Should Save: eval_acc={results['eval_acc']}, best_acc={best_acc}")
        ...
        if args.evaluate_during_training:
            ...
            eval_result = evaluate(args, model, tokenizer, eval_when_training=True)
...
def evaluate(args, model, tokenizer, eval_when_training=False):
    if args.evaluate_during_training:
        eval_dataloader = DataLoader(args.val_dp, batch_size=args.eval_batch_size, pin_memory=True)
    if args.do_test:
        eval_dataloader = DataLoader(args.test_dp, batch_size=args.eval_batch_size, pin_memory=True)
    ...
    logger.info("  Num examples = %d", len(args.test_dp))
    ...
    for batch in bar:
        ...
        code, _labels = batch['function_body'], batch['label']
        input_sentence = tokenizer(code, padding='longest', truncation=True, max_length=512)
        ids = torch.tensor(input_sentence['input_ids'])
        ids = ids.to(args.device)
        mask = torch.tensor(input_sentence['attention_mask']).bool()
        ...
        with torch.no_grad():
            logit = model(input_ids=ids, attention_mask=mask)
...
def main():
    ...
    # # 仅取前 N 个样本用于调试运行（如 N = 50）
    # args.train_dp = torch.utils.data.Subset(args.train_dp, list(range(500)))
    # args.val_dp = torch.utils.data.Subset(args.val_dp, list(range(200)))
    ...
    if args.do_train:
        ...
        model_name = args.model_name.split("/")[1]
        results_prefix = f"{model_name}_alpha_{args.alpha}_{args.seed}_layers_{args.layers}_results.json"

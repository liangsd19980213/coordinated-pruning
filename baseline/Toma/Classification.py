...
def feature_extraction_all(file_path):
    features = []
    labels = []
    with open(file_path, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            try:
                feature_vec = [float(row[f"t{i}_sim"]) for i in range(1, 7)]
                label = int(row["Label"])
...
def random_features(features, labels, sample_num=20000):
    positives = [(f, l) for f, l in zip(features, labels) if l == 1]
    negatives = [(f, l) for f, l in zip(features, labels) if l == 0]

    pos_sample = random.sample(positives, min(sample_num, len(positives)))
    neg_sample = random.sample(negatives, min(sample_num, len(negatives)))

    all_samples = pos_sample + neg_sample
    random.shuffle(all_samples)

    final_features = [s[0] for s in all_samples]
    final_labels = [s[1] for s in all_samples]
    return final_features, final_labels


# ========== 3. 分类器评估 ==========

def evaluate_classifier(X, y, classifier, n=5):
    ...
    print(f"\n===== {n}-Fold Evaluation Results ({classifier.__class__.__name__}) =====")
...
def train_and_test(train_file, test_file, classifier):
    X_train, y_train = feature_extraction_all(train_file)
    X_test, y_test = feature_extraction_all(test_file)

    classifier.fit(X_train, y_train)
    y_pred = classifier.predict(X_test)

    print(f"\n===== Final Test Results ({classifier.__class__.__name__}) =====")
...
def main():
    ...
    parser.add_argument('--mode', type=str, default='cv', choices=['cv', 'test'], help="cv: cross-validation; test: final evaluation")
    parser.add_argument('--classifier', type=str, default='voting', choices=['lr', 'dt', 'svc', 'nb', 'voting'])
    ...
    if args.mode == 'cv':
        # 读取并合并 train + valid
        train_path = "./output/bcb_train_sim.csv"
        valid_path = "./output/bcb_valid_sim.csv"
        train_X, train_y = feature_extraction_all(train_path)
        valid_X, valid_y = feature_extraction_all(valid_path)
        features = train_X + valid_X
        labels = train_y + valid_y
        evaluate_classifier(features, labels, clf, n=args.folds)

    elif args.mode == 'test':
        # 最终训练：train + valid 训练，test 测试
        train_path = "./output/bcb_train_sim.csv"
        valid_path = "./output/bcb_valid_sim.csv"
        test_path = "./output/bcb_test_sim.csv"
        merged_train_path = "./output/merged_train_valid.csv"
        ...
        if not os.path.exists(merged_train_path):
            ...
            df_all.to_csv(merged_train_path, index=False)

        train_and_test(merged_train_path, test_path, clf)
